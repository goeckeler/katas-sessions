...
// ========================================================
    ...
    public void ex01_accumulateFirstLetters() {
        ...
        String result = input.stream().map(s -> s.substring(0, 1)).collect(Collectors.joining());
    ...
    public void ex04_stringifyMap() {
        ...
        String result = input.entrySet().stream().map(e -> e.getKey() + e.getValue()).collect(Collectors.joining());
        // By Stefanie and Alexander
        // StringBuffer sb = new StringBuffer();
        // input.forEach((k,v) -> sb.append(k).append(v));
...
// ========================================================
    ...
    public void ex06_upcaseOddLengthWords() {
        ...
        List<String> result = input.stream().filter(s -> s.length() % 2 == 1).map(s -> s.toUpperCase())
    ...
    public void ex07_joinStreamRange() {
        ...
        String result = input.stream().skip(1).limit(4).map(s -> s.substring(1, 2)).collect(Collectors.joining(","));
    ...
    public void ex10_findLongestLine() throws IOException {
        // String longest = reader.lines().max((s1, s2) -> Integer.compare(s1.length(), s2.length())).get();
        // By Thorsten
    ...
    public void ex11_selectByLengthAndPosition() {
        ...
        List<String> result = IntStream.range(0, input.size()).filter(i -> input.get(i).length() > i)
    ...
    public void ex12_listDifference() {
        ...
        List<Integer> result = IntStream.range(0, one.size()).mapToObj(i -> one.get(i) - two.get(i))
                .collect(Collectors.toList());
...
// ========================================================
    ...
    public void ex13_stringsToCharacters() {
        ...
        List<Character> result = input.stream().flatMap(s -> s.chars().mapToObj(i -> (char) i))
                .collect(Collectors.toList());
    ...
    public void ex14_listOfAllWords() throws IOException {
        List<String> output = reader.lines().flatMap(line -> Arrays.stream(line.split(REGEXP)))
                .collect(Collectors.toList());
    ...
    public void ex15_longLowerCaseSortedWords() throws IOException {
        List<String> output = reader.lines().flatMap(line -> Arrays.stream(line.split(REGEXP)))
                .filter(s -> s.length() >= 8).map(s -> s.toLowerCase()).sorted().collect(Collectors.toList());
    ...
    public void ex16_longLowerCaseReverseSortedWords() throws IOException {
        List<String> result = reader.lines().flatMap(line -> Arrays.stream(line.split(REGEXP)))
                .filter(s -> s.length() >= 8).map(s -> s.toLowerCase()).sorted(Comparator.reverseOrder())
                .collect(Collectors.toList());
    ...
    public void ex17_sortedLowerCaseDistinctByLengthThenAlphabetically() throws IOException {
        List<String> result = reader.lines().flatMap(line -> Arrays.stream(line.split(REGEXP)))
                ...
                .sorted(Comparator.comparing((String s) -> s.length()).thenComparing(Comparator.naturalOrder()))
                .collect(Collectors.toList());
    ...
    public void ex18_countTotalAndDistinctWords() {
        ...
        long distinctCount = reader.lines().flatMap(line -> Arrays.stream(line.split(REGEXP)))
                .peek(w -> totalCnt.addAndGet(1)).map(s -> s.toLowerCase()).distinct().count();
        long totalCount = totalCnt.get();
...
// ========================================================
    ...
    public void ex19_bigFactorial() {
        BigInteger result = LongStream.range(2, 22).mapToObj(n -> BigInteger.valueOf(n)).reduce(BigInteger.ONE,
                (a, b) -> a.multiply(b));
    ...
    public void ex20_getLastWord() throws IOException {
        String result = reader.lines().flatMap(line -> Arrays.stream(line.split(REGEXP))).reduce(null, (s1, s2) -> s2);
    ...
    public void ex21_mapLengthToWordList() throws IOException {
        Map<Integer, List<String>> result = reader.lines().flatMap(line -> Arrays.stream(line.split(REGEXP)))
    ...
    public void ex23_wordFrequencies() throws IOException {
        // Solution 1
        // Map<String, Long> result = reader.lines().flatMap(line -> Arrays.stream(line.split(REGEXP)))
